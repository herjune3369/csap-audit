...
# -*- coding: utf-8 -*-
"""
모의 LLM 파이프라인 통합 실행 스크립트
네트워크 문제 없이 전체 시스템을 테스트할 수 있는 모의 버전
"""
...
def run_step2_generate_prompts(
...
async def run_step3_call_mock_llm(
    prompts: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    """3단계: 모의 LLM API 호출"""
    print("🔄 3단계: 모의 LLM API 호출 시작...")
    ...
    # 결과 저장
    output_file = "output/mock_llm_responses.json"
...
def run_step4_generate_excel(results: List[Dict[str, Any]], system_type: str = "Linux"):
    ...
    with open(temp_json_file, "w", encoding="utf-8") as f:
        json.dump({"results": enhanced_items}, f, ensure_ascii=False, indent=2)

    output_path = generator.generate_csap_excel(temp_json_file)

    print(f"✅ 4단계 완료: Excel 보고서 생성 - {output_path}")


async def main():
    """메인 실행 함수"""
    try:
        print("🚀 모의 LLM 파이프라인 시작")
        print("=" * 50)
        ...
        # 3단계: 모의 LLM API 호출
        results = await run_step3_call_mock_llm(prompts)

        # 4단계: Excel 보고서 생성
        run_step4_generate_excel(results, system_type)

        print("=" * 50)
        print("🎉 모의 LLM 파이프라인 완료!")
        print("\n📁 생성된 파일들:")
        print("- output/llm_prompts.json")
        print("- output/mock_llm_responses.json")
