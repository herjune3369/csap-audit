    ...
    for i, prompt_data in enumerate(test_prompts, 1):
        ...
        try:
            ...
            response = await asyncio.wait_for(
                model.generate_content_async(prompt_text), timeout=60
            )
            ...
            results.append({"CCE_ID": cce_id, "success": True, "data": llm_data})

        except Exception as e:
            print(f"❌ {cce_id} 처리 실패: {e}")
            results.append({"CCE_ID": cce_id, "success": False, "error": str(e)})
