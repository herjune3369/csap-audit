...
class LLMCaller:
    ...
    async def call_llm(self, prompt: str) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(f"LLM API 호출 시도 {attempt + 1}/{max_retries} (타임아웃: {timeout}초)")
                ...
                logger.warning(f"LLM 호출 타임아웃 (시도 {attempt + 1}/{max_retries}, {timeout}초 초과)")
    ...
    async def process_prompts_batch(self, prompts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for i, prompt_data in enumerate(prompts, 1):
            ...
            logger.info(f"⏱️  예상 남은 시간: {estimated_remaining // 60}분 {estimated_remaining % 60}초")
