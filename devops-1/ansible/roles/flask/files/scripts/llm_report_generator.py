...
2. 조치방법:
   - 실제 {system} 시스템에서 따라할 수 있는 구체적인 단계별 조치 방법
...
"""
    ...
    def generate_excel_report(self, enhanced_items: List[Dict[str, Any]], system_name: str) -> str:
        ...
        for row, item in enumerate(enhanced_items, 2):
            worksheet.cell(row=row, column=1, value=row-1).border = self.borders['thin']  # No
            worksheet.cell(row=row, column=2, value=item.get('시스템', system_name)).border = self.borders['thin']
            worksheet.cell(row=row, column=3, value=item.get('분류', '')).border = self.borders['thin']
            worksheet.cell(row=row, column=4, value=item.get('CCE_ID', '')).border = self.borders['thin']
            worksheet.cell(row=row, column=5, value=item.get('점검항목', '')).border = self.borders['thin']
            
            # 중요도
            importance_cell = worksheet.cell(row=row, column=6, value=item.get('중요도', ''))
            ...
            worksheet.cell(row=row, column=8, value=item.get('현황', '')).border = self.borders['thin']
            worksheet.cell(row=row, column=9, value=item.get('개선방안', '')).border = self.borders['thin']
            worksheet.cell(row=row, column=10, value=item.get('상세해설', '')).border = self.borders['thin']
            ...
            worksheet.cell(row=row, column=11, value=action_text).border = self.borders['thin']
    ...
    def generate_summary_sheet(self, workbook: Workbook, items: List[Dict[str, Any]], system_name: str):
        """요약 통계 시트 생성"""
